<x-filament-panels::page>
    @php
        // برچسب‌های فارسیِ اولویت — جایگزینِ ucfirst()ِ انگلیسی
        $priorityLabels = ['low' => 'کم', 'medium' => 'متوسط', 'high' => 'زیاد', 'critical' => 'بحرانی'];
    @endphp

    <style>
        .cp-wrap{direction:rtl}
        ...
        .cp-column-dot{width:9px;height:9px;border-radius:50%;display:inline-block;margin-inline-end:.4rem}
        ...
        .cpcal-day{min-height:110px;border-inline-start:1px solid rgb(229 231 235);border-bottom:1px solid rgb(229 231 235);padding:.35rem;display:flex;flex-direction:column;gap:.25rem;transition:background .15s}
        :root.dark .cpcal-day{border-color:#374151}
        .cpcal-day:nth-child(7n+1){border-inline-start:none}
        ...
        .cp-table th{text-align:start;padding:.5rem .6rem;background:#f9fafb;border-bottom:1px solid rgb(229 231 235);font-weight:600;color:#374151;white-space:nowrap}
    ...
    <div class="cp-wrap" dir="rtl">
        <div class="cp-tabs">
            <button type="button" wire:click="setView('kanban')" class="{{ $activeView === 'kanban' ? 'active' : '' }}">کانبان</button>
            <button type="button" wire:click="setView('calendar')" class="{{ $activeView === 'calendar' ? 'active' : '' }}">تقویم</button>
            <button type="button" wire:click="setView('table')" class="{{ $activeView === 'table' ? 'active' : '' }}">جدول</button>
            <button type="button" wire:click="setView('dashboard')" class="{{ $activeView === 'dashboard' ? 'active' : '' }}">داشبورد</button>
        </div>

        @if($activeView !== 'dashboard')
            <div class="cp-filters">
                <input type="text" wire:model.live.debounce.400ms="search" placeholder="جستجوی عنوان…">

                <select wire:model.live="filterStage">
                    <option value="all">همه‌ی مراحل</option>
                ...
                <select wire:model.live="filterAuthor">
                    <option value="all">همه‌ی نویسندگان</option>
                ...
                <select wire:model.live="filterTag">
                    <option value="all">همه‌ی برچسب‌ها</option>
                    @foreach($this->filterTags as $id => $title)
                ...
                <select wire:model.live="filterPublicationStatus">
                    <option value="all">همه‌ی وضعیت‌های انتشار</option>
                    @foreach(\App\Filament\Pages\ContentPlanner::PUBLICATION_STATUSES as $key => $label)
                        <option value="{{ $key }}">{{ $label }}</option>
                    @endforeach
                </select>

                <button type="button" class="reset" wire:click="resetFilters">پاک‌کردن فیلترها</button>
            </div>
        @endif
                            ...
                            <div class="cp-card" draggable="true" data-plan-id="{{ $plan->id }}">
                                ...
                                <div class="cp-card-row">
                                    ...
                                    @foreach($plan->tags as $tag)
                                        <span class="cp-badge">{{ $tag->title }}</span>
                                    @endforeach
                                </div>

                                <div class="cp-card-row">
                                    <span class="cp-badge">{{ $plan->author?->name ?? '—' }}</span>
                                    <span class="cp-badge cp-priority-{{ $plan->priority }}">{{ $priorityLabels[$plan->priority] ?? $plan->priority }}</span>
                                </div>

                                @php($score = $this->scoreCardFor($plan))
                                @if($score)
                                    <div class="cp-scores">
                                        <span>سئو {{ $score['categories']['seo']['score'] }}</span>
                                        <span>AI {{ $score['overall'] }}</span>
                                        <span>خوانایی {{ $score['categories']['readability']['score'] }}</span>
                                    </div>
                                @endif

                                <div class="cp-card-footer">
                                    <span>{{ optional($plan->effectivePublishDate())->format('Y-m-d') ?? 'بدون تاریخ' }}</span>
                                    <span>{{ $plan->updated_at->diffForHumans() }}</span>
                                </div>

                                <div class="cp-card-row">
                                    <a href="{{ $this->editUrlFor($plan) }}" wire:navigate>ویرایش ←</a>
                                    @if($plan->contentable_id)
                                        <a href="{{ $this->aiAssistantUrlFor($plan) }}" wire:navigate>دستیارِ هوشِ مصنوعی ←</a>
                                    @else
                                        <button type="button" class="cp-ai-btn" wire:click="generateDraft({{ $plan->id }})">✨ تولیدِ پیش‌نویس</button>
                                    @endif
                                </div>
                            </div>
                        @empty
                            <div class="cp-empty">کارتی نیست</div>
                        @endforelse
                    </div>
                @empty
                    <p class="cp-placeholder">هیچ مرحله‌ی خط‌تولیدی تعریف نشده است.</p>
                @endforelse
            </div>

            @if(count($selectedPlanIds))
                <div class="cp-bulk-bar">
                    <span>{{ count($selectedPlanIds) }} کارت انتخاب شده</span>
                    ...
                    <button type="button" wire:click="bulkMoveStage">انتقال</button>
                    ...
                    <button type="button" wire:click="bulkSetPriority">تنظیمِ اولویت</button>

                    <button type="button" class="danger" wire:click="bulkDelete" wire:confirm="{{ count($selectedPlanIds) }} کارت حذف شود؟ این کار هیچ مقاله/صفحه‌ی مرتبطی را حذف نمی‌کند.">حذف</button>
                </div>
            @endif
        @elseif($activeView === 'calendar')
            <div class="cpcal-toolbar">
                <div class="cpcal-nav">
                    <button type="button" wire:click="previousMonth">→ ماهِ قبل</button>
                    <button type="button" wire:click="goToday">امروز</button>
                    <button type="button" wire:click="nextMonth">ماهِ بعد ←</button>
                    <span class="cpcal-title">{{ $this->monthLabel }}</span>
                </div>
                <div class="cpcal-legend">
                    <span><i class="cpcal-dot content"></i> مقاله/صفحه</span>
                    <span><i class="cpcal-dot planned"></i> ایده‌ی برنامه‌ریزی‌شده</span>
                    <span><i class="cpcal-dot deadline"></i> مهلتِ پیش‌نویس</span>
                </div>
            </div>

            <div class="cpcal-grid" id="contentPlannerCalendar" wire:loading.class="opacity-50">
                @foreach(['یک‌شنبه', 'دوشنبه', 'سه‌شنبه', 'چهارشنبه', 'پنج‌شنبه', 'جمعه', 'شنبه'] as $d)
                    ...
                    <tbody>
                        @forelse($this->tablePlans as $plan)
                            ...
                            <tr>
                                ...
                                <td><span class="cp-badge cp-priority-{{ $plan->priority }}">{{ $priorityLabels[$plan->priority] ?? $plan->priority }}</span></td>
                        ...
                        @empty
                            <tr><td colspan="14" class="cp-empty">هیچ برنامه‌ی محتوایی با این فیلترها مطابقت ندارد.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        @elseif($activeView === 'dashboard')
            <div class="cp-stats">
                <div class="cp-stat"><span class="cp-stat-value">{{ $this->dashboardStats['ideas'] }}</span><span class="cp-stat-label">ایده‌ها</span></div>
                <div class="cp-stat"><span class="cp-stat-value">{{ $this->dashboardStats['drafts'] }}</span><span class="cp-stat-label">پیش‌نویس‌ها</span></div>
                <div class="cp-stat"><span class="cp-stat-value">{{ $this->dashboardStats['reviews'] }}</span><span class="cp-stat-label">در حالِ بازبینی</span></div>
                <div class="cp-stat"><span class="cp-stat-value">{{ $this->dashboardStats['scheduled'] }}</span><span class="cp-stat-label">زمان‌بندی‌شده</span></div>
                <div class="cp-stat"><span class="cp-stat-value">{{ $this->dashboardStats['published'] }}</span><span class="cp-stat-label">منتشرشده</span></div>
                <div class="cp-stat"><span class="cp-stat-value">{{ $this->dashboardStats['avg_publishing_days'] ?? '—' }}</span><span class="cp-stat-label">میانگینِ روزها تا انتشار</span></div>
                <div class="cp-stat"><span class="cp-stat-value">{{ $this->dashboardStats['avg_review_days'] ?? '—' }}</span><span class="cp-stat-label">میانگینِ روزها در بازبینی</span></div>
            </div>

            <div class="cp-chart-card">
                <h3>تولیدِ محتوا در هر ماه</h3>
                @php($maxCount = max(1, collect($this->productionPerMonth)->max('count')))
                <div class="cp-bars">
                    @foreach($this->productionPerMonth as $month)
                        <div class="cp-bar-col">
                            <div class="cp-bar" style="height:{{ $month['count'] > 0 ? max(6, round(($month['count'] / $maxCount) * 100)) : 2 }}px" title="{{ $month['count'] }} منتشرشده"></div>
