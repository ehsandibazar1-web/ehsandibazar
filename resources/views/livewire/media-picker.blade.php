{{--
    پنجره‌ی انتخابِ رسانه — یک‌بار سراسری در چرومِ پنل mount می‌شود. تا وقتی isOpen نیست
...
>
    <style>
        ...
        .mp-folders{border-inline-end:1px solid rgb(229 231 235);padding:.75rem;overflow-y:auto}
        :root.dark .mp-folders{border-color:#3f3f46}
        .mp-folder{display:flex;align-items:center;gap:.4rem;width:100%;text-align:start;background:none;border:none;cursor:pointer;padding:.4rem .5rem;border-radius:.5rem;font-size:.82rem;color:#374151}
        ...
        .mp-item{border:2px solid transparent;border-radius:.6rem;overflow:hidden;background:#f3f4f6;cursor:pointer;text-align:start;padding:0;position:relative;aspect-ratio:1/1}
        ...
        .mp-item .badge{position:absolute;top:.25rem;inset-inline-start:.25rem;background:#f59e0b;color:#fff;border-radius:9999px;width:1.1rem;height:1.1rem;display:flex;align-items:center;justify-content:center;font-size:.65rem;font-weight:700}
        ...
        .mp-panel{border-inline-start:1px solid rgb(229 231 235);overflow-y:auto;padding:1rem}
        ...
        .mp-usage{font-size:.76rem;color:#92400e;margin:.2rem 0 0;padding-inline-start:1.1rem}
    ...
    @if($isOpen)
        <div class="mp-overlay" wire:key="mp-overlay" x-on:keydown.escape.window="$wire.close()">
            <div class="mp-modal" dir="rtl" x-on:click.outside="$wire.close()">
                {{-- هدر: عنوان + جست‌وجو + تعویضِ نما + بستن --}}
                <div class="mp-head">
                    <h2>{{ $onlyImages ? 'انتخابِ تصویر' : 'کتابخانه‌ی رسانه' }}</h2>
                    <input type="search" class="mp-search" wire:model.live.debounce.350ms="search"
                        placeholder="جست‌وجو در نامِ فایل، ALT، کپشن، توضیح، نوع…">
                    <div class="mp-viewtoggle">
                        <button type="button" class="{{ $viewMode === 'grid' ? 'active' : '' }}" wire:click="setViewMode('grid')" title="نمای شبکه‌ای">▦</button>
                        <button type="button" class="{{ $viewMode === 'list' ? 'active' : '' }}" wire:click="setViewMode('list')" title="نمای فهرستی">☰</button>
                    </div>
                    <button type="button" class="mp-close" wire:click="close" title="بستن">✕</button>
                </div>

                {{-- فیلترها: در حالتِ فقط-تصویر چیپ‌های نوع پنهان می‌شوند --}}
                <div class="mp-filters">
                    @unless($onlyImages)
                        @php($types = ['all' => 'همه','image' => 'تصاویر','video' => 'ویدئوها','document' => 'اسناد','audio' => 'صوت','archive' => 'آرشیوها','other_only' => 'سایر'])
                    ...
                    @endunless
                    <button type="button" class="mp-chip {{ $onlyUnused ? 'active' : '' }}" wire:click="$toggle('onlyUnused')">استفاده‌نشده</button>
                </div>
                ...
                <div class="mp-body {{ $this->selectedMedia ? 'with-panel' : '' }}">
                    ...
                    <div class="mp-folders">
                        <button type="button" class="mp-folder {{ is_null($currentFolderId) ? 'active' : '' }}" wire:click="openFolder(null)">📁 همه‌ی فایل‌ها</button>
                    ...
                    <div class="mp-main">
                        ...
                        <div class="mp-drop"
                            ...
                            <strong>فایل را بکشید و رها کنید</strong> یا برای آپلود کلیک کنید — به‌طور خودکار با WebP، تامبنیل و اندازه‌های ریسپانسیو به کتابخانه اضافه می‌شود.
                            <span wire:loading wire:target="uploads">· در حالِ آپلود…</span>
                        ...
                        @if($search === '' && $this->breadcrumbTrail)
                            <div class="mp-breadcrumb">
                                <button type="button" wire:click="openFolder(null)">کتابخانه‌ی رسانه</button>
                            ...
                            <div class="mp-grid" wire:loading.class="opacity-50" wire:target="search,typeFilter,onlyUnused,openFolder,setTypeFilter">
                                @forelse($this->mediaItems as $item)
                                    <button type="button" wire:key="mp-g-{{ $item->id }}"
                                        ...
                                        title="{{ $item->original_name }} — برای استفاده دابل‌کلیک کنید">
                                        @if($item->type === 'image')
                                ...
                                @empty
                                    <div class="mp-empty" style="grid-column:1/-1">هنوز فایلی اینجا نیست — از بالا آپلود کنید.</div>
                                @endforelse
                            </div>
                        @else
                            <div class="mp-list" wire:loading.class="opacity-50" wire:target="search,typeFilter,onlyUnused,openFolder,setTypeFilter">
                                ...
                                @empty
                                    <div class="mp-empty">هنوز فایلی اینجا نیست — از بالا آپلود کنید.</div>
                                @endforelse
                            </div>
                        @endif
                    </div>
                    ...
                    @if($this->selectedMedia)
                        ...
                        <div class="mp-panel" wire:key="mp-panel-{{ $sel->id }}">
                            @if($sel->processingFailed())
                                <div class="mp-note err">✕ پردازش نشد — نسخه‌ی بهینه (WebP) ندارد. ممکن است خراب یا پشتیبانی‌نشده باشد.</div>
                            @endif
                            ...
                            <div class="kv">
                                ...
                                آپلود در {{ $sel->created_at?->format('Y/m/d') }}
                                @if($sel->folder) · در {{ $sel->folder->fullPath() }}@endif
                            </div>
                            ...
                            @if($sel->type === 'image')
                                <div class="mp-fld">
                                    <label>متنِ جایگزین (ALT) — دسترس‌پذیری و سئوی تصویر</label>
                                    <input type="text" x-ref="mpAlt" value="{{ $sel->alt_text }}" placeholder="این تصویر را توصیف کنید…">
                                    <div style="margin-top:.35rem"><x-filament::button size="xs" color="gray" x-on:click="$wire.saveAltText($refs.mpAlt.value)">ذخیره‌ی ALT</x-filament::button></div>
                                </div>
                            @endif

                            <div class="mp-fld">
                                <label>کپشن</label>
                                <input type="text" x-ref="mpCap" value="{{ $sel->caption }}" placeholder="یک کپشنِ کوتاه…">
                                <div style="margin-top:.35rem"><x-filament::button size="xs" color="gray" x-on:click="$wire.saveCaption($refs.mpCap.value)">ذخیره‌ی کپشن</x-filament::button></div>
                            </div>

                            <div class="mp-fld">
                                <label>توضیحات</label>
                                <input type="text" x-ref="mpDesc" value="{{ $sel->description }}" placeholder="یک توضیحِ بلندتر…">
                                <div style="margin-top:.35rem"><x-filament::button size="xs" color="gray" x-on:click="$wire.saveDescription($refs.mpDesc.value)">ذخیره‌ی توضیحات</x-filament::button></div>
                            </div>

                            @if($sel->type === 'image')
                                <div class="mp-fld">
                                    <label>بهینه‌سازیِ WebP</label>
                                    @if($sel->webp_path)
                                        <div class="mp-note ok" style="margin-bottom:.35rem">
                                            اصلی: <strong>{{ $sel->human_size }}</strong> · WebP: <strong>{{ $sel->webp_human_size }}</strong>
                                            @php($saved = $sel->webp_savings_percent)
                                            @if(! is_null($saved))
                                                ·
                                                @if($saved >= 0)
                                                    <strong>{{ $saved }}%</strong> صرفه‌جویی
                                                @else
                                                    {{ abs($saved) }}% بزرگ‌تر
                                                @endif
                                            @endif
                                        ...
                                        @if($sel->responsive_urls)
                                            <div class="kv" style="margin-bottom:.35rem">اندازه‌های ریسپانسیو: {{ implode('px, ', array_keys($sel->responsive_urls)) }}px</div>
                                        @endif
                                    @else
                                        <div class="mp-note warn" style="margin-bottom:.35rem">هنوز نسخه‌ی WebP ندارد.</div>
                                    @endif
                                    <x-filament::button size="xs" color="gray" icon="heroicon-o-arrow-path" wire:click="regenerateDerivatives({{ $sel->id }})" wire:loading.attr="disabled" wire:target="regenerateDerivatives">بازتولیدِ WebP</x-filament::button>
                                </div>
                            @endif

                            <div class="mp-fld">
                                <label>استفاده‌شده در ({{ count($usages) }})</label>
                                @if(count($usages))
                                    <ul class="mp-usage">
                                        @foreach($usages as $usage)<li>{{ $usage['type'] }} — {{ $usage['label'] }} ({{ $usage['field'] }})</li>@endforeach
                                    </ul>
                                @else
                                    <div class="kv">هنوز جایی استفاده نشده است.</div>
                                @endif
                            </div>

                            <div style="display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.5rem">
                                <x-filament::button size="xs" color="gray" tag="a" :href="$sel->url" target="_blank">دانلود</x-filament::button>
                            </div>
                        </div>
                    @endif
                </div>
                ...
                <div class="mp-foot">
                    <span class="hint">تک‌کلیک انتخاب می‌کند · دابل‌کلیک بلافاصله استفاده می‌کند</span>
                    <div style="display:flex;gap:.5rem">
                        <x-filament::button color="gray" wire:click="close">انصراف</x-filament::button>
                        @if($selectedMediaId)
                            <x-filament::button wire:click="chooseAndReturn({{ $selectedMediaId }})">استفاده از این فایل</x-filament::button>
                        @else
                            <x-filament::button disabled>استفاده از این فایل</x-filament::button>
                        @endif
                    </div>
                </div>
            </div>
