...
@section('site.css')
    ...
    @if(isset($maxPrice) && !empty($maxPrice))
        <link rel="stylesheet" type="text/css" href="{{ url('') }}/site_theme/css/nouislider.min.css"/>
    @endif
    ...
    <div class="container site-blog__box">
        @if(isset($title))
            <div class="row search-result-header">
                <!--Search result text-->
                <h4>{{ count($products) }} products were found for {{ $title }}</h4>
            </div>
        @endif
        <div class="row">
        ...
        <div class="row" >
            <div class="search-product">
               ...
               @include('site.product.partials.filter',compact('category'))
                    ...
                    <h3>{{ isset($category) && $category != false ? $category->title : ' products '.env('SITE_NAME_FA') }}</h3>
                </div>
                       </div>
				...
				<div class="image-holder">
					...
					<div class="image-liquid image-holder--original" style="background-image: url('{{ isset($product->image[0]) && !empty($product->image[0]) ? url($product->image[0]->url) : null }}')">
					</div>
				</div>
					...
					<div class="row">
						...
						<div class="col-12 col-sm-12 product-description__price">
                                                        ...
                                                        <span class="offer-mob">
                                                                <span class="off">{{ \App\Utility\DiscountType::showCentDiscount($product) }}</span>
                                                        </span>
                                                        @endif
                                                        <br>
					...
					<div class="sizes-wrapper">
						...
						<span class="secondary-text text-uppercase">
							<ul class="list-inline p-0 text-center">
								...
								<li>
								     <a href="javascript:void(0)" class="add-crd" title="مقایسه" onclick="compare('{{$product->id}}')">
                                       ...
                                       {{--  <div class="item position-relative">
                                                ...
                                                <div class="img-pro">
                                                    ...
                                                    <a href="{{ $product->path() }}" class="d-block">
                                                        ...
                                                        <img src="{{ isset($product->image[0]) && !empty($product->image[0]) ? url($product->image[0]->url) : null }}" class="img-fluid" alt="{{ $product->title }}">
                                                    </a>
                                                </div>
                        ...
                        <div class="loadmore-result text-center">
                            <button class="btn btn-waiting load-more">more</button>
                            <input type="hidden" class="countProduct"
                                   value="{{ isset($countProduct) ? $countProduct : 0 }}">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="brand-sliders-item">
                    <div class="row brand-slider-header">
                        <div class="col-12 p-0 text-center">
                             <h4 class="title-section text-center wow fadeInUp">favorites</h4>
                        </div>
                    </div>
...
@section('site-js')
    ...
    @if(isset($maxPrice) && !empty($maxPrice))
        ...
        <script src="{{ url('') }}/site_theme/js/nouislider.min.js"></script>
