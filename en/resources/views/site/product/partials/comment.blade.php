...
<div class="ask-your-q">
    @guest()
        <a href="javascript:void(0)">
            To register a comment, you must first log in to your account
        </a>
@endguest
        ...
        <form action="{{ route('site.add.comment',isset($article) ? $article->id : $product->id) }}" class="form"
            ...
            <div class="form-group">
                ...
                <textarea name="comment" cols="30" rows="10" class="form-control"
                          placeholder="Write your comment or question for us ..."></textarea>
                @if($errors->has('comment'))
                    <span class="text-danger">{{ $errors->first('comment') }}</span>
                @endif
            </div>
            <button class="btn btn-site">Send</button>
        </form>
    @else
                    ...
                    <div class="reply-question-item">
                        ...
                        <div class="reply-main-detail">
                            ...
                            <p class="reply-dependies">
                                ...
                                <span>in history :  @php $v = verta($comment->created_at);
                                            echo $v->format('%d %B %Y');
                                    @endphp</span>
                            </p>
                        </div>
