...
class SiteController extends Controller
{
    ...
    public function index()
    {
        ...
        OpenGraph::setDescription($seo->title);
        OpenGraph::setTitle($seo->description);
    ...
    public function categorys($slug = null)
    {
        if (!empty($slug)) {
            ...
            if (isset($category) && !empty($category)) {
                ...
                where('parent_id', $category->id)->latest()->get();
    ...
    public function sellerProfile($id = null)
    {
        if (!empty($id)) {
            ...
            if (isset($user) && !empty($user)) {
                ...
                $categorys = Category::query()->whereType(Product::class)->with(['products.variations'])->latest()->get();
    ...
    public function article($slug = null)
    {
        if (!empty($slug)) {
            ...
            if ($article) {
                ...
                $categorys = Category::query()->whereType(Article::class)->whereStatus(1)->latest()->get();
                createMetaSite($article);
                SEO::opengraph()->setUrl(Url(''));
                OpenGraph::addImage(['url' => Url($article->image[0]->url), 'size' => 300]);
            ...
            $articles = Article::with('image')->where('status', 1)->paginate(8);
            $categorys = Category::whereType(Article::class)->whereStatus(1)->latest()->get();
