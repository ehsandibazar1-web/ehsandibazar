...
class OrderController extends Controller
{
        ...
        if (isset($id) && isset($status) && is_numeric($id) && is_numeric($status) && !empty($id) && !empty($status)) {
            ...
            if (isset($findOrderItem)) {
                foreach ($findOrderItem as $itemOrderItem) {
                    ...
                    $this->changeStatusSendSms($status, $variation, $findOrder->user, $findOrder->tracking_code, $unserialize['qty']);
                }
            }
    ...
    public function shippingCode(Request $request, $id)
    {
        ...
        if ($updateOrder) {
            SendSms::sms([$trackingCode,$code], 98702, $order->user->mobile);
    ...
    private function changeStatusSendSms($status_id, $variation, $user, $tracking_code, $qty)
    {
        switch ($status_id) {
            case Status::WAITING:
                 ...
                 SendSms::sms([$user->name,$tracking_code,'در انتظار تایید'], 98025, $user->mobile);
                // event(new changeStatusSendSms($user, Status::WAITING, $tra
                break;
            case Status::PENDING:
...
//                ]);
                ...
                SendSms::sms([$user->name,$tracking_code,'در حال پردازش'], 98025, $user->mobile);
                // event(new changeStatusSendSms($user, Status::PENDING, $tracking_code));
                break;
            case Status::SENDING:
                  
                  SendSms::sms([$user->name,$tracking_code,'ارسال شد'], 98025, $user->mobile);
                // event(new changeStatusSendSms($user, Status::PAID, $tracking_code));
                break;
            case Status::PAID:
              
                SendSms::sms([$user->name,$tracking_code,'تسویه شده'], 98025, $user->mobile);
                // event(new changeStatusSendSms($user, Status::PAID, $tracking_code));
                break;
            case Status::UNPAID:
                ...
                SendSms::sms([$user->name,$tracking_code,'پرداخت نشده'], 98025, $user->mobile);
                // event(new changeStatusSendSms($user, Status::PAID, $tracking_code));
                break;
            case Status::CANCELED:
                ...
                SendSms::sms([$user->name,$tracking_code,'لغو شده'], 98025, $user->mobile);
            ...
            case Status::RETURNED:
                ...
                SendSms::sms([$user->name,$tracking_code,'مرجوعی'], 98025, $user->mobile);
