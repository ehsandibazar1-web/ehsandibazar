        ...
        <div class="col-lg-12">
            <section class="panel padding-bottom-panel">
                ...
                <header class="panel-heading">
                    @lang('cms.header-list-tag')
                    <a type="button" class="btn btn-xs btn-success top-left" data-toggle="modal"
                       href="{{route('panel.tag.create')}}">@lang('cms.create-new-item')
                    </a>
                </header>
                        ...
                        @if(isset($tags) && count($tags) > 0)
                            @foreach($tags as $val)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('panel.tag.status' , ['id' => $val->id])}}">{{  \App\Utility\Status::getStatus($val->status)  }}</a>
                                    </td>

                                    <td>

                                        <a class="btn btn-warning btn-xs" title="@lang('cms.edit')"
                                           href="{{ route('panel.tag.edit',['id'=>$val->id ]) }}"><i
                                                class="icon-edit "></i></a>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <form action="{{ route('panel.tag.delete' , ['id' => $val->id])  }}"
                                      method="POST">
