            ...
            <div class="row">
                <div class="profile-page col-xl-9 col-lg-8 col-md-12 order-2">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="col-12">
                                <h1 class="title-tab-content text-danger"> Please be patient until the files are uploaded</h1>
                            </div>
                                        ...
                                        <div class="col-12">
                                            @if(isset($product->video) && !empty($product->video))
                                                @foreach($product->video as $key => $video)
                                                    @if($key > 0)
                                                        <span>
                                                            <h5>  title {{ $key }} : {{ $video->title }}</h5>
                                                            @if(isset($product->catalog[$key]) && !empty($product->catalog[$key]))
                                                                <a href="{{ $key == 0 ? asset('public'.$product->catalog[$key]->url) : $product->catalog[$key]->url }}"
                                                                   target="_blank">Download Help PDF</a>
                                                            @endif
                                                        </span>
                                                        <div>
                                                            {!! $video->url !!}
                                ...
                                @elseif(isset($product->type) && $product->type == \App\Utility\ProductType::VOICE)
                                    @if(isset($product->video[1]) && !empty($product->video[1]))
                                        <div class="AudioPlayerLayout">
                                            ...
                                            @endif
                                            @else
                                                <div class="uk-alert uk-alert-warning">Unfortunately the book is not available!
                                                </div>
                                            @endif
                                        </div>
                            </div>
