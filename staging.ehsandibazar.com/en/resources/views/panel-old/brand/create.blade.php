        ...
        <div class="col-lg-12">
            <section class="panel">
                ...
                <header class="panel-heading">
                    @if(isset($findBrand) )
                        @lang('cms.header-brand-edit')
                    @else
                        @lang('cms.header-brand-create')
                    @endif

                    <a type="button" class="btn btn-xs btn-info pull-left top-left" data-toggle="modal"
                       href="{{route('panel.brand.index')}}">@lang('cms.back')
                    </a>
                    <br>
                    <br>
                </header>
                    ...
                    <div class=" form">
                        ...
                        @if(isset($findBrand) )
                            <form class="cmxform form-horizontal tasi-form" id="commentForm" method="post"
                                  ...
                                  action="{{route('panel.brand.update' , ['id' => $findBrand->id])}}">
                                {{method_field("PATCH")}}
                                @else
                                    <form class="cmxform form-horizontal tasi-form" id="commentForm" method="post"
                                          enctype="multipart/form-data"
                                        ...
                                        <div class="tab-content">
                                            <div id="main" class="tab-pane fade in active">
                                                @include('panel-old.brand.partials.main')
                                            </div>

                                            <div id="gallery" class="tab-pane fade">
                                                @include('panel-old.brand.partials.gallery')
                                            </div>
                                        </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            /* when edit product mode */
                ...
                @if(isset($findBrand) && !empty($findBrand) && count($findBrand->image) > 0)
            var count = 5;
            @foreach($findBrand->image as $key => $itemImages)
