        ...
        <div class="col-lg-12">
            <section class="panel padding-bottom-panel">
                ...
                <header class="panel-heading">
                   @lang('cms.list-multi-attribute')
                    <a type="button" class="btn btn-xs btn-success" data-toggle="modal"
                       href="{{route('panel.attribute-type.create')}}"> @lang('cms.create-new-item')
                    </a>
                </header>
            </section>
        </div>
        ...
        <div class="container">
            <section class="panel">
                <div class="panel-body">
                        ...
                        @if(isset($attributeType) && count($attributeType) > 0)
                            @foreach($attributeType as $val)
                                <tr>
                                  ...
                                  {{--  <td> {{ $val->lang  }} </td>--}}
                                    <td>
                                        <a href="{{route('panel.attribute-type.status' , ['id' => $val->id])}}">{{  \App\Utility\Status::getStatus($val->status)  }}</a>
                                    </td>

                                    <td>

                                        <a class="btn btn-warning btn-xs" title="@lang('cms.edit')"
                                           href="{{ route('panel.attribute-type.edit',['id'=>$val->id ]) }}"><i
                                                class="icon-edit "></i></a>
                    ...
                    @if(isset($attributeType) && count($attributeType) > 0)
                        <span style="margin-right: 45%">{!! $attributeType->render() !!}</span>
                    @endif
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <form action="{{ route('panel.attribute-type.delete' , ['id' => $val->id])  }}"
                                      method="POST">
