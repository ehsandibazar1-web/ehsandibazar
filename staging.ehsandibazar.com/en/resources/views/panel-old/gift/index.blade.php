            ...
            <section class="panel">
                <div class="panel-body">
                    <table class="table table-hover">
                        ...
                        <tbody>
                        @if(isset($allGift) && count($allGift) > 0)
                            @foreach($allGift as $val)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ str_limit($val->name , 20) }}</td>
                                    <td> {{ $val->product->title  }} </td>
                                    <td> {{ $val->score  }} </td>
                                    <td>
                                        <a href="{{route('panel.gift.request.status' , ['id' => $val->id])}}">{{  \App\Utility\Status::getStatus($val->status)  }}</a>
                                    </td>

                                    <td>

                                        <a class="btn btn-warning btn-xs" title="@lang('cms.edit')"
                                           href="{{ route('panel.gift.edit',['id'=>$val->id ]) }}"><i
                                                class="icon-edit "></i></a>
                    ...
                    @if(isset($allGift) && count($allGift) > 0)
                        <span style="margin-right: 45%">{!! $allGift->render() !!}</span>
                    @endif
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <form action="{{ route('panel.gift.delete' , ['id' => $val->id])  }}" method="POST">
