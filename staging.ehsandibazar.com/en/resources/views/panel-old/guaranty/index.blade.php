        ...
        <div class="col-lg-12">
            <section class="panel padding-bottom-panel">
                ...
                <header class="panel-heading">
                    @lang('cms.header-guaranty-list')
                    <a type="button" class="btn btn-xs btn-success top-left" data-toggle="modal"
                       href="{{route('panel.guaranty.create')}}">@lang('cms.create-new-item')
                    </a>
                </header>
            ...
            <section class="panel">
                <div class="panel-body">
                    <table class="table table-hover">
                        ...
                        <tbody>
                        @if(isset($allGuaranty) && count($allGuaranty) > 0)
                            @foreach($allGuaranty as $val)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ str_limit($val->title , 20) }}</td>
                                    <td>
                                        <a href="{{route('panel.guaranty.status' , ['id' => $val->id])}}">{{  \App\Utility\Status::getStatus($val->status)  }}</a>
                                    </td>

                                    <td>

                                        <a class="btn btn-warning btn-xs" title="@lang('cms.edit')"
                                           href="{{ route('panel.guaranty.edit',['id'=>$val->id ]) }}"><i
                                                class="icon-edit "></i></a>

                                        <a class="btn btn-primary btn-xs" title="@lang('cms.show')"
                                           data-toggle="modal"
                                           href="#show{{$val->id}}"><i
                    ...
                    @if(isset($allGuaranty) && count($allGuaranty) > 0)
                        <span style="margin-right: 45%">{!! $allGuaranty->render() !!}</span>
                    @endif
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h4 class="modal-title" style="font-family: yekan"><span
                                        class="">@lang('cms.show-item')</span>
                                </h4>
                            </div>
                            <div class="modal-body text-center">
                                ...
                                @if(!empty($val->description))
                                    ...
                                    {!! $val->description !!}
                                @else
                                    <p class="alert alert-info border-right-info">@lang('cms.no-description')</p>
                                @endif
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <form action="{{ route('panel.guaranty.delete' , ['id' => $val->id])  }}" method="POST">
