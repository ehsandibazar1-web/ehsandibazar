...
class BasketZarinPalController extends Controller
{
    ...
    public function finishBasket(Request $request)
    {
        ...
        $this->validate($request, [
            'payment' => "required|integer|between:0,3",
        ...
        if($sessionBasket && !empty($sessionBasket)){
            foreach($sessionBasket->items as $id=>$sessionb){
                if(($sessionb['type']== \App\Utility\ProductType::SIMPLE)){
                    $this->sessionAddress=true;
        ...
        $saveItem = [
            ...
            'user_info' => isset($sessionAddress) ? serializeAndUnSerialize::serializeAndUnSerializeInfoUser($this->user, $sessionAddress) : '',
...
//              if (isset($totalDiscount) && $totalDiscount >= 1 && !Auth::user()->isColleague()) {
                ...
                if (isset($orderItemCreate) && !empty($orderItemCreate)) {
                    ...
                    if (isset($totalPrice) && $totalPrice > 0){
                        ...
                        foreach ($orderCreate->orderItem as $orderItem) {
                            ...
                            if ((in_array($product->type, [ProductType::VOICE, ProductType::PDF, ProductType::VIDEO])) || (isset($product->video) && count($product->video) > 1) || (isset($product->catalog) && count($product->catalog) > 1)) {
