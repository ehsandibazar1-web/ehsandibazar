...
/**
 * یک تماسِ API هزینه‌بر است — مثلِ RunAiContentGeneration فقط یک‌بار تلاش می‌کند، بدونِ retryِ خودکار.
 * پیامِ کاربر را طبقه‌بندی می‌کند (ContentAssistantService::classifyIntent) و بر اساسِ نتیجه یکی از
 * سه کار را می‌کند: یک AiGenerationِ معمولی صف می‌کند (همان RunAiContentGenerationِ موجود)، یک
 * ترجمه‌ی کامل صف می‌کند (همان TranslateArticleDraft)، یا صرفاً یک پاسخِ گفتگویی assistant می‌نویسد.
 * در هر سه حالت هیچ مسیرِ تولیدِ جدیدی ساخته نمی‌شود؛ فقط همان‌هایی که از قبل موجودند صدا زده می‌شوند.
 */
class ProcessAiChatMessage implements ShouldQueue
{
    ...
    public function handle(ContentAssistantService $service): void
    {
        ...
        try {
            ...
            $this->reply("متأسفم، خطایی رخ داد: {$e->getMessage()}");
