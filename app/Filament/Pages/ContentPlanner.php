...
/**
 * مرکزِ اصلیِ خط‌تولید محتوا — چهار نما (Kanban/Calendar/Table/Dashboard) روی یک صفحه، با یک نوار
 * فیلتر/جستجوی مشترک، دقیقاً طبق همان الگوی «یک صفحه، چند تب» که SeoCenter قبلاً استفاده کرده —
 * نه چند آیتم جدا در نویگیشن. پورت از سایتِ انگلیسی، تطبیق‌یافته به اسکیمای فارسی:
 * Article/Page زیر App\Model، User زیر App\، Tag با ستونِ title، statusِ بولین برای فیلترِ انتشار،
 * localeِ fa/en. اعلان‌های وابسته به NotificationPreference (که در فارسی وجود ندارد) حذف شده‌اند؛
 * فقط توست‌های استانداردِ UIِ Filament باقی مانده‌اند. هیچ منطقِ هوشِ مصنوعیِ تازه‌ای اضافه نشده.
 */
class ContentPlanner extends Page
{
    ...
    protected static ?string $title = 'برنامه‌ریزِ محتوا';
    ...
    // پارامتر آدرس ?view=calendar اجازه می‌دهد لینک‌های قدیمی (مثلاً EditorialCalendar) دقیقاً
    ...
    private function baseQuery(): Builder
    {
        return ContentPlan::query()
            ...
            ->when($this->filterPublicationStatus !== 'all', function (Builder $q) {
                $value = $this->filterPublicationStatus;
                ...
                // تطبیقِ فارسی: Article/Page ستونِ statusِ بولین دارند (۱ منتشر، ۰ پیش‌نویس) به‌علاوه‌ی
                // published_at. whereHasMorph نوعِ مورفی را از roiِ morphMap (article/page) حل می‌کند،
                // پس پاس‌دادنِ کلاسِ مدل درست است. STAGE قبلیِ رشته‌ای انگلیسی اینجا معنا ندارد.
                $q->whereHasMorph('contentable', [Article::class, PageModel::class], function ($sub) use ($value) {
                    match ($value) {
                        'draft' => $sub->where('status', 0),
                        'published' => $sub->where('status', 1),
                        'scheduled' => $sub->whereNotNull('published_at')->where('published_at', '>', now()),
                        default => $sub,
                    };
                });
            })
    ...
    {
        // تطبیقِ فارسی: App\Model\Tag ستونِ نمایشی‌اش title است، نه name
        return Tag::orderBy('title')->pluck('title', 'id');
    }
    ...
    // هرگز مستقیماً چیزی تولید نمی‌کند (تولید همیشه با کلیک صریح ادمین داخل AiAssistantPanel است) —
    // این متدها فقط رکورد را (در صورت نیاز) مادیت می‌بخشند و ادمین را به صفحه‌ی ویرایش/دستیاری می‌برند.

    /**
     * «تولیدِ پیش‌نویس» روی یک کارتِ بدونِ contentable — رکورد Article/Page را می‌سازد (اگر نبود)،
     * مرحله را به AI Draft می‌برد، و ادمین را مستقیماً به صفحه‌ی ویرایش هدایت می‌کند.
     */
    public function generateDraft(int $planId): void
    {
        ...
        if (! $plan->contentable_id) {
            Notification::make()->danger()->title('ساختِ پیش‌نویس برای این ایده ممکن نشد')->send();
    ...
    // لینک مستقیم به دستیارِ هوشِ مصنوعیِ همین رکورد (صفحه‌ی مستقلِ AiContentAssistant) — null اگر هنوز
    // contentable ندارد (یعنی باید دکمه‌ی «تولیدِ پیش‌نویس» نشان داده شود، نه این). تطبیقِ فارسی:
    // contentable_type با aliasِ مورفیِ کوچک (article/page) ذخیره می‌شود.
    public function aiAssistantUrlFor(ContentPlan $plan): ?string
    {
        ...
        return strtolower($plan->contentable_type) === 'page'
    ...
    public function moveCard(int $planId, int $stageId): void
    {
        ...
        Notification::make()->success()->title("انتقال به «{$stage->label}»")->send();
    }
    ...
    public function bulkMoveStage(): void
    {
        ...
        Notification::make()->success()->title("{$count} کارت به «{$stage->label}» منتقل شد")->send();
    }
    ...
    public function bulkSetPriority(): void
    {
        ...
        Notification::make()->success()->title("اولویتِ {$count} کارت به‌روزرسانی شد")->send();
    }
    ...
    public function bulkDelete(): void
    {
        ...
        Notification::make()->success()->title("{$count} کارت حذف شد")->send();
    }

    // ============ Calendar — جانشینِ EditorialCalendar، همان منطقِ ماه/هفته، گسترش‌یافته برای ============
    // Page + پین‌های planned/deadlineِ کارت‌های برنامه‌ریز
    ...
    // تطبیقِ فارسی: نوعِ ورودی می‌تواند aliasِ مورفیِ کوچک (article/page) یا برچسبِ صریحِ بلید
    // (Article/Page) باشد — با strtolower هر دو حالت پوشش داده می‌شوند.
    public function contentEditUrl(string $type, int $id): string
    {
        return strtolower($type) === 'page'
            ? PageResource::getUrl('edit', ['record' => $id])
            : ArticleResource::getUrl('edit', ['record' => $id]);
    }

    /**
     * فراخوانی از سمت جاوااسکریپت هنگام رهاکردنِ یک چیپ روی روزِ جدید — فقط تاریخ عوض می‌شود
     * (ساعت قبلی حفظ می‌شود)، دقیقاً همان رفتارِ EditorialCalendar::moveArticle() قبلی.
     */
    ...
    {
        $model = strtolower($type) === 'page' ? PageModel::find($id) : Article::find($id);
        ...
        Notification::make()->success()->title('به '.$newDate.' منتقل شد')->send();
    }
    ...
    private function reschedulePlanned(int $id, string $newDate): void
    {
        ...
        Notification::make()->success()->title('تاریخِ برنامه‌ریزی‌شده به '.$newDate.' منتقل شد')->send();
    }
    ...
    private function rescheduleDeadline(int $id, string $newDate): void
    {
        ...
        Notification::make()->success()->title('مهلت به '.$newDate.' منتقل شد')->send();
    }

    // ============ Dashboard — همه از content_plans/content_plan_stage_transitions موجود، بدون جدولِ تازه ============
    ...
    // میانگینِ زمان از ساختِ کارت تا رسیدن به Published (روز) — فقط اولین ورود به این مرحله به‌ازای هر کارت
    ...
    // میانگینِ زمانِ توقف در مرحله(های) داده‌شده (روز) — فقط بازدیدهایی که به مرحله‌ی بعدی رسیده‌اند
