 ...
 * از همان مکانیزم LogsActivity که Article استفاده می‌کند.
 *
 * توجه (نسخه‌ی فارسی): مسیر ایندکسِ برداری (RAG/embedding) فعلاً یک stub است —
 * App\Services\Rag\IndexingService یک no-op است و هیچ تماس شبکه‌ای/embedding انجام نمی‌دهد.
 * قلاب‌های چرخه‌ی عمر همچنان جاب را دیسپچ می‌کنند تا وقتی embedding واقعی آمد، بدون تغییرِ مدل
 * فعال شود (forward-compatible).
 */
class KnowledgeEntry extends Model
{
    ...
    // فقط وقتی content واقعاً عوض شده یا ورودی تازه ساخته شده دوباره صف می‌شود. فعلاً IndexingService
    // یک stub است، پس این دیسپچ یک no-op بی‌ضرر است (هیچ تماس embedding‌ای انجام نمی‌شود).
    ...
    // قطعه‌های خودِ فیلد content این ورودی (chunkable_type=KnowledgeEntry)
