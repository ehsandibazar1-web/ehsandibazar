...
1. **کلیدِ API روی سرور:** باید `ANTHROPIC_API_KEY` در `.env`ِ سرور (یا env سیپنل) تنظیم شود.
   بدونِ آن، `NullProvider` فعال است و تولید کار نمی‌کند (خطا نمی‌دهد، فقط خروجیِ خالی). **هزینه:**
   هر تولید، مصرفِ API آنتروپیک دارد (پولی). اول روی استگینگ با کلید تست می‌کنیم.
